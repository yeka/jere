...
function readTriad($hundredths, $tenths, $ones, $powerOf3 = false)
{
    $words = [];
    $num = $hundredths;
    if ($num == 1) {
...
function translateIt($inputNumber)
{
    $string = "$inputNumber";
    $digit = strlen($string);
    ...
    if ($lastOp !== 0) {                            // === Evaluating the front non 'multiple of 3' digits
        ...
        $temp = readTriad($hundredths, $tenths, $ones, $powerOf3);
        if ($numOfOp > 0) {
    ...
    for ($i = $numOfOp; $i >= 1; $i--) {            // === Evaluating per 3 digit
        ...
        $power = "e".(3 * ($i - 1));
        $temp = readTriad($hundredths, $tenths, $ones);
        if ($power !== "e0") {
            $temp[] = $power;
...
function readIt($inputCode, $wordLib)
{
    $sentence = "";
    foreach ($inputCode as $key => $code) {
        $sentence .= $wordLib[$code];
        if ($code != 'o1') {
            $sentence .= " ";
        }
    }
